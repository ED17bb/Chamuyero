...
export default function ElImpostorApp() {
  ...
  useLayoutEffect(() => {
    ...
    if (!document.getElementById('google-font')) {
      ...
      link.id = 'google-font';
      link.rel = 'stylesheet';
      link.href = "https://fonts.googleapis.com/css2?family=Nunito:wght@400;700;900&family=Rye&display=swap"; // Añadida fuente Rye para estilo Wanted
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{__html: `
        ...
        /* ESTILO WANTED NEWSPAPER */
        .wanted-filter {
            filter: grayscale(100%) contrast(120%) brightness(1.1) sepia(0.4);
        }
        .wanted-paper {
            background-color: #f4e4bc;
            background-image: url("data:image/svg+xml,%3Csvg width='4' height='4' viewBox='0 0 4 4' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 3h1v1H1V3zm2-2h1v1H3V1z' fill='%23d1bfa3' fill-opacity='0.4' fill-rule='evenodd'/%3E%3C/svg%3E");
        }
        .font-wanted {
            ...
            <div className="absolute inset-0 z-[100] bg-black flex flex-col items-center justify-center p-4">
              <div className="w-full max-w-sm bg-slate-900 rounded-3xl border border-slate-700 overflow-hidden shadow-2xl">
                 ...
                 <div className="relative aspect-square bg-black overflow-hidden">
                    ...
                    <div className="absolute inset-0 border-[40px] border-black/50 rounded-full z-10 pointer-events-none"></div>
                 </div>
                        ...
                        <div className="flex -space-x-3">
                           {players.slice(0, 3).map(p => (
                             <div key={p.id} className="w-10 h-10 rounded-full border-2 border-slate-800 bg-[#e3d0b1] overflow-hidden relative">
                                {p.photo ? (
                                  <img src={p.photo} alt={p.name} className="w-full h-full object-cover wanted-filter" />
                                ) : (
                        ...
                        <div key={player.id} className="flex items-center gap-2 animate-fade-in">
                            <div className="flex-1 relative flex items-center gap-2">
                                ...
                                >
                                   {player.photo ? (
                                     <>
                                       <img src={player.photo} alt={player.name} className="w-full h-full object-cover wanted-filter" />
                                       <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-20">
                                          <Camera size={20} className="text-white" />
                                       </div>
                                     </>
                                   ) : (
                                     <div className="w-full h-full flex items-center justify-center text-slate-500 hover:text-purple-700">
                                       <Camera size={24} />
                                     </div>
                                   )}
                                </button>
              ...
              <div className="flex-1 flex flex-col items-center justify-center w-full">
                  <div className="perspective-1000 w-72 h-72 cursor-pointer group mb-6" onClick={handleFlipCoin}>
                      <div className={`relative w-full h-full duration-700 transform-style-3d transition-transform ${isCoinFlipped ? 'rotate-y-180' : ''}`}>
                        ...
                        {/* CARA FRONTAL (INCÓGNITA CON FOTO WANTED) */}
                        <div className={`absolute w-full h-full backface-hidden rounded-full border-8 border-slate-800 shadow-[0_0_50px_rgba(0,0,0,0.5)] flex items-center justify-center wanted-paper overflow-hidden`}>
                            {players[currentPlayerIndex].photo ? (
                              <div className="flex flex-col items-center justify-between w-full h-full p-4 relative z-10">
                                  {/* Encabezado WANTED */}
                                  <div className="text-center w-full border-b-2 border-black/60 pb-1 mb-1">
                                      <h3 className="font-wanted text-2xl text-black tracking-widest leading-none">SE BUSCA</h3>
                                  </div>
                                  
                                  {/* Foto central con filtro */}
                                  <div className="flex-1 w-full border-2 border-black/40 p-1 bg-black/5 mx-auto overflow-hidden relative">
                                      <img 
                                        src={players[currentPlayerIndex].photo} 
                                        alt="Player" 
                                        className="w-full h-full object-cover wanted-filter grayscale contrast-125" 
                                      />
                                  </div>

                                  {/* Pie de foto RECOMPENSA */}
                                  <div className="text-center w-full pt-1 mt-1 border-t-2 border-black/60">
                                      <p className="font-wanted text-lg text-black font-bold tracking-tighter">RECOMPENSA</p>
                                      <p className="font-sans text-xs font-bold text-black/70">$10.000</p>
                                  </div>
                              </div>
                            ) : (
